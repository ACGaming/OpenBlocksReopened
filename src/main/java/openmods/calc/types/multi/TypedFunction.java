...
import openmods.calc.utils.AnnotationMap;
import openmods.calc.utils.DefaultMap;
import openmods.calc.utils.OptionalInt;
import openmods.calc.utils.Stack;
import openmods.calc.utils.reflection.TypeVariableHolder;
import openmods.calc.utils.reflection.TypeVariableHolderFiller;
	...
	private static TypeVariant createVariant(final Method method) {
		...
		for (int i = 0; i < parameterCount; i++) {
			...
			if (isVariadicArg) {
				...
				final Class<?> varType = TypeVariableHolders.OptionalType.resolve(type);
				if (isRawArg) {
